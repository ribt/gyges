...
void firstphase(board game){
	...
	for(int k = 0; k < NB_PLAYERS; k++){	//Does the same for both players
		...
		int pawn_size = 0;
		if(k == 0){printf("Joueur Sud, veuillez saisir un par un, de gauche à droite le placement de vos pions puis valider avec entrée.\n");
		}else{printf("Joueur Nord, veuillez saisir un par un,de gauche à droite le placement de vos pions puis valider avec entrée.\n");}
